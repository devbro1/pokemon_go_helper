...
async function getScreenshotData(filename = "screenshot.png") {
    ...
    rc["cp"] = await Tesseract.recognize("cp.png",{psm: 13, oem: 3, tessedit_char_whitelist: '0123456789'});
...
async function outputData() {
    ...
    console.log(
        ...
        spokemon.cp,
        ",",
        ((spokemon.ind.atk + spokemon.ind.def + spokemon.ind.sta) / 45).toFixed(
            2
        ),
        ",",
        spokemon.ind.atk,
        ...
        spokemon.ind.sta,
        ",",
        (current.little.stat_product / best.little?.stat_product).toFixed(2),
        ",",
        (current.great.stat_product / best.great?.stat_product).toFixed(2),
        ",",
        (current.ultra.stat_product / best.ultra?.stat_product).toFixed(2)
    );
}

async function main() {
    console.log("name, CP, IV,atk,def,sta,little,great,ultra");
